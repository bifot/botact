...
class Botact {
  constructor(settings) {
    ...
    if (!confirmation) {
      throw new Error('You must set confirmation code in settings')
    } else if (typeof confirmation !== 'string') {
      throw new Error('Confirmation code must be a string')
    } else if (!token) {
      throw new Error('You must set token in settings')
    } else if (typeof token !== 'string') {
      throw new Error('Token must be a string')
    } else if (group_id !== undefined && typeof group_id !== 'number') {
