...
class Botact {
  ...
  handler (data) {
    ...
    if (this.messages.commands[data.text.toLowerCase()] !== undefined) {
      this.messages.commands[data.text.toLowerCase()](data)
    } else {
      ...
      Object.keys(methods).some((key, i) => {
        if (new RegExp(key, 'i').test(data.text.toLowerCase())) {
  ...
  listen (req, res) {
    ...
    if (req.body.type === 'message_new') {
      ...
      data.text = getLastMessage(data).body

      delete data.user_id
      delete data.body

      if (this.settings.sub !== undefined) {
        isMember(this.settings.group_id, data.uid)
          .then(() => this.handler(data))
          .catch(() => this.reply(data.uid, this.settings.sub))
      } else {
        this.handler(data)
      }
    }
